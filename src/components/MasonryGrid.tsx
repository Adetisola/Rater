...
export function MasonryGrid({ postIds, isLoading, maxColumns }: MasonryGridProps) {
  ...
  return (
    <div className="w-full max-w-[2600px] mx-auto px-2 xs:px-2 md:px-4 pb-20 relative">
      ...
      <div className={`flex gap-2 xs:gap-4 items-start justify-center text-left transition-opacity duration-300 ${isLoading && postIds.length === 0 ? 'opacity-0' : 'opacity-100'}`}>
