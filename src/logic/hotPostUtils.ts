...
export async function computeHotPosts(posts: Post[]): Promise<Set<string>> {
  ...
  const recentEligible = posts.filter(post => {
    ...
    const reviewCount = post.post_metrics?.review_count ?? 0;
    return postAge <= SEVEN_DAYS_MS && reviewCount >= MIN_REVIEW_COUNT;
  });
  ...
  const sorted = [...recentEligible].sort(
    (a, b) => (b.post_metrics?.review_count ?? 0) - (a.post_metrics?.review_count ?? 0)
  );
